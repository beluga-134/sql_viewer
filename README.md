...
- 打开 DuckDB 时批量读取字段名但不统计行数，点击表后才统计行数
- 原生查询限制为 2 GB 内存、4 个线程，超限工作数据写入系统临时目录
- 单次结果最多返回 100,000 行，可取消正在执行的桌面查询
- 展示字段名、DuckDB 类型、文件大小和行数
- 读取 DuckDB 原生列注释，并在字段列表和结果表头显示最多 20 字的说明
- 左侧数据源按文件、模式、表/视图的目录树展示，支持按表名和字段名筛选
- 记录最近打开的 10 个 DuckDB 数据库路径，可从标题栏快速重新打开
- 支持持久化项目目录：新建项目目录后输入实际文件夹路径，自动递归扫描其中的 DuckDB 数据库
- SQL 查询与本地查询历史
- 结果表格固定表头和原始行号，支持按列输入内容进行包含筛选
- 导出当前完整查询结果为 XLSX
...
可以直接使用 DuckDB 列注释提供字段说明，无需额外的元数据表：

```sql
COMMENT ON COLUMN main.orders.customer_id IS '客户唯一编号';
```
